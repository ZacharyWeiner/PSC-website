...
import MenuComponent2Vue from '../../../components/MenuComponent2.vue';
//import { useStore } from "vuex";
import { useGameScores } from "./../../../services/firebase.js"
export default {
    components: {MenuComponent2Vue},
    setup () {
        const state = reactive({
            count: 0,
        })
        let {getHighScores} = useGameScores();
        let {scoresForGame} = getHighScores("ColorBlaster");
        return {
            ...toRefs(state), scoresForGame
        }
    },
    mounted(){
        this.playGame(this);
    ...
    methods: {
        playGame(page){
            ...
            saveGameScore: function (score){
                console.log(page.$store.state.relayx_handle, score);
                console.log("adding gamescore",  score, page.$store.state.relayx_handle, page.$store.state.user_address);
                let { addGameScore } = useGameScores();
                addGameScore("ColorBlaster", score, page.$store.state.relayx_handle, page.$store.state.user_address);
            },
