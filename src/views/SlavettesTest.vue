    ...
    <div id="Slavettes" class="w-full grid grid-cols-1 md:grid-cols-4 xl:grid-cols-4 mt-20 p-6 text-white">
        <div class="col-span-2">
            ...
            <div class="rounded-2xl overflow-hidden text-left ring-2 ring-pink-400 m-2 p-4 overflow-y-scroll">
                <div  v-if="userSlavettes.length === 0" class="text-7xl font-extrabold text-center">
...
export default {
    ...
    async setup() {
        ...
        if(totalMoves.value > 4 ){totalMoves = 4}
        ...
        return { userSlavettes, jackpots, addjackpot, imgSrc, redCount, goldCount, greenCount, meta, undressing, loadingMessage, showConfirm, undressed, totalMoves, showBuyButton, movesToBuy, reloadOnChange }
    },
    mounted() {
      if(this.userSlavettes && this.userSlavettes.length > 0){
    ...
    methods: {
        ...
        checkJackpot(slavetteId) {
            ...
            let setterDate = comparison.getDate() -7
            ...
            if ((hasPhone || this.goldCount > 0) && (this.availableMoves >= 4)) {
                ...
                let existing = this.jackpots.filter(jp => jp.slavetteId === slavetteId)
                if(existing.length > 0){
                    let newestObj = existing.reduce((a, b) => {
                        return new Date(a.createdAt.seconds*1000) > new Date(b.createdAt.seconds*1000) ? a : b;
                    });
                    let newest = new Date(newestObj.createdAt.seconds * 1000)               
                    console.log(`The date of slavette ${slavetteId}'s last egg was made on: ${newest.toLocaleDateString("EN")} by ${newestObj.userName}`)
                    if(newest <= comparison){
                        console.log("JACKPOT!")
                        getsJackpot = true
                    }
                }else{
        ...
        countdown(slavette){
            let newestObj;
            if(this.jackpots){
                let exists = this.jackpots.filter(jp => jp.slavetteId == slavette.props.no)
                console.log(`slavette ${slavette.props.no}'s has made ${exists.length} eggs, ${exists.length == 0 ? "if 0 the page may be loading" : ""}`)
                if(exists.length > 0){
                    newestObj = exists.reduce((a, b) => {
                        return new Date(a.createdAt.seconds*1000) > new Date(b.createdAt.seconds*1000) ? a : b;
                    });
                }
                if(newestObj){
            ...
            // console.log("The page may be loading or there are no jackpots to find for this slavette")
            return "now"
                    
        },
        initBuyButton(){
            let amount = this.movesToBuy * 0.000123
            let _local = this;
             window.relayone.render(this.$refs['buyExtra'], {
                to: "gopnikdevfund@relayx.io",
                amount: amount,
                currency: "BSV",
                onPayment: () => {
